...
class Block:
    ...
    final block which is most likely smaller than the REQUEST_SIZE.
    """
    Missing = 0
    Pending = 1
    Retrieved = 2
...
class Piece:
    """
    The piece is a part of the torrent's content. Each piece except the final
    ...
    data between peers a smaller unit is used - this smaller piece is referred
    ...
    def block_received(self, offset: int, data: bytes):
        ...
        if block:
            ...
            #Trying to complete a non-existing block 
            pass
...
class PieceManager:
    ...
    def __init__(self, torrent):
        ...
        self.fd = os.open(self.torrent.root_folder, os.O_RDWR | os.O_CREAT)
    ...
    def complete(self) -> bool:
        """
        Checks whether or not all the pieces are downloaded for this torrent.
        """
    ...
    def add_peer(self, peer_id, bitfield):
        """
        Adds a peer and the bitfield representing the pieced the peer has.
        """
    ...
    def next_request(self, peer_id) -> Block:
        ...
        #one. TODO: replace this with an implementation of 
        # "rarest-piece-first" algorithm instead
        #
        #The algorithm tries to download the pieces in sequence and will try to finish
        #started pieces before starting with new pieces.
        #
    ...
    def block_received(self, peer_id, piece_index, block_offset, data):
        ...
        #piece that the received block belongs to
        piece = pieces[0] if pieces else None
        if piece:
            piece.block_received(block_offset, data)
            if piece.is_complete():
                if piece.is_hash_matching():
                    #if all blocks for piece received and the hash matches
                    #write the piece to file, and update status
                    self._write(piece)
                    ...
                    print('{} / {} pieces donwloaded {} %'
                          .format(complete, self.total_pieces, (complete/self.total_pieces)*100))
                else:
                    print('Discarding corrupt piece {}'.format(piece.index))
                    piece.reset()
        else:
    ...
    def _expired_requests(self, peer_id) -> Block:
        """
        Go through previously requested blocks, if any one has been in the
        ...
        for request in self.pending_blocks:
            if self.peers[peer_id][request.block.piece]:
                if request.added + self.max_pending_time < current:
                    print('Re-requesting block {} for piece {}'.format(
    ...
    def _next_ongoing(self, peer_id) -> Block:
        ...
        for piece in self.ongoing_pieces:
            if self.peers[peer_id][piece.index]:
                #Are there any blocks left to request in this piece?
